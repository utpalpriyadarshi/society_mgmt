...
def check_bank_entries():
    \"\"\"Check the bank statement entries.\"\"\"
    try:
        ...
        print('ID\tDate\t\tDescription\t\t\tAmount\t\tBalance\t\tRef No\t\tStatus\t\tMatched Ledger')
        print('-' * 120)
        for entry in entries:
            print(f'{entry[0]}\t{entry[1]}\t{entry[2][:30]:<30}\t{entry[3]}\t\t{entry[4]}\t\t{entry[5][:10]:<10}\t{entry[7]:<12}\t{entry[8] or \"None\"}')
