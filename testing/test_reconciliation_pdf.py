#!/usr/bin/env python3
"""
Test script to verify the reconciliation feature implementation with PDF support
"""
...
def test_reconciliation_feature():
    ...
    csv_entries = [
        {
            'date': '2023-01-15',
            'description': 'Maintenance payment from A101',
            'amount': 500.0,
            'balance': 1500.0,
        ...
        {
            'date': '2023-01-20',
            'description': 'Maintenance payment from B202',
            'amount': 500.0,
            'balance': 2000.0,
    ...
    csv_imported = bank_manager.import_statement(csv_entries, "test_user")
    print(f"CSV import result: {csv_imported} entries imported")
    
    # Retrieve entries to verify
    entries = bank_manager.get_all_entries()
    print(f"Total entries after CSV import: {len(entries)}")
